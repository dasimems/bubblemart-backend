...
const MAX_RETURN_PRODUCT_COUNT = 20;
...
const getProductController: ControllerType = async (req, res) => {
  ...
  try {
    ...
    const maxPage = Math.ceil(totalProducts / MAX_RETURN_PRODUCT_COUNT) || 1;
    ...
    const skip = MAX_RETURN_PRODUCT_COUNT * (formattedPage - 1);
    const fetchedProduct = await ProductSchema.find()
      .skip(skip)
      .limit(MAX_RETURN_PRODUCT_COUNT)
      .exec();
